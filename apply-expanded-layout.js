...
const searchBlock = `                {/* 3 Bottom Widgets */}
...
const newBlock = `                {/* 3 Bottom Widgets */}
                <div className="flex flex-col gap-6 w-full mb-8">
                  ...
                  <div className="bg-[#111520] border border-[#1f2947] rounded-xl p-6 shadow-xl flex flex-col w-full h-[350px]">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <span className="text-xs bg-[#1f2947] text-slate-300 px-3 py-1.5 rounded-md border border-[#2a3454] uppercase font-bold tracking-wider">Last 4 Weeks</span>
                    </div>
                    <div className="flex-1 min-h-0 w-full mt-2">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={displayWeeklyData} margin={{ top: 15, right: 0, left: -25, bottom: 0 }}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#1f2947" vertical={false} />
                          <XAxis dataKey="name" stroke="#64748b" tick={{ fill: '#94a3b8', fontSize: 12, fontWeight: 'bold' }} tickMargin={12} axisLine={false} tickLine={false} />
                          ...
                          <Bar dataKey="suppliers" fill="#10b981" radius={[4, 4, 0, 0]} maxBarSize={60}>
                            <LabelList dataKey="suppliers" position="top" fill="#10b981" fontSize={13} fontWeight="bold" formatter={(v: any) => '£' + v.toLocaleString()} offset={8} />
                          </Bar>
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
...
const regexFriendlySearchStr = escapeRegExp(searchBlock).replace(/\\s+/g, '\\\\s+');

const regex = new RegExp(regexFriendlySearchStr);
...
if (regex.test(content)) {
    ...
    const searchNoCR = searchBlock.replace(/\\r/g, '');
    const regex2 = new RegExp(escapeRegExp(searchNoCR).replace(/\\s+/g, '\\\\s+'));
    if (regex2.test(content)) {
        content = content.replace(regex2, newBlock);
        fs.writeFileSync('app/dashboard/HenleyDashboard.tsx', content);
        console.log('Layout updated successfully (Linux CR).');
    } else {
