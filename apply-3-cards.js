...
const dataCalc = `  const customExpensePieData = [
...
content = content.replace(`  const getExportFilename = () => {`, dataCalc);

// 2. Add UI
const uiBlock = `                        <Bar yAxisId="right" dataKey="orders" fill="#f97316" radius={[4, 4, 0, 0]} maxBarSize={60}>
...
const searchStr = `                        <Bar yAxisId="right" dataKey="orders" fill="#f97316" radius={[4, 4, 0, 0]} maxBarSize={60}>
                          <LabelList dataKey="orders" position="top" fill="#f97316" fontSize={12} fontWeight="bold" offset={8} />
                        </Bar>
                      </ComposedChart>
                    </ResponsiveContainer>
...
content = content.replace(searchStr, uiBlock);

fs.writeFileSync('app/dashboard/HenleyDashboard.tsx', content);
console.log('Added 3 bottom widgets.');
