...
export function HenleyDashboard({ is2025 = false }: { is2025?: boolean }) {
  ...
  const [offerForm, setOfferForm] = useState({ platform: 'just_eat', store: 'Herbies Pizza', amount: '', startDate: '', type: 'weekly', notes: '' })
  ...
  useEffect(() => {
    const loadReport = () => {
      ...
      // Load offers
      fetch(`/api/offers?clientId=${clientId}&is2025=${is2025 ? 'true' : 'false'}`, { cache: 'no-store' })
        .then(r => r.json())
  ...
  }, [session, filter, store, platform, is2025])
  ...
  const realOffersData = offers
    .filter(o => {
      const storeMatch = !store || store === 'Combined' || o.store === store || o.store === 'Combined';
    ...
    return {
      ...
      platform: (PLATFORM_LABELS[offer.platform] || offer.platform) + (offer.store !== 'Combined' ? ` (${offer.store})` : ''),
      notes: offer.notes,
      spend: offer.amount,
      orders: totalOrders,
  ...
  const customExpensePieData = [
    { name: 'Wages', value: staffWages },
    { name: 'Supplier Purchases', value: totalSuppliers },
    { name: 'Franchise & POS', value: franchiseFees + totalCommission },
    { name: 'Others', value: otherExpenses },
    { name: 'Utilities', value: utilities },
    { name: 'Marketing', value: marketing + adSpends }
  ].filter(e => e.value > 0);
    ...
    <div className="space-y-8 pb-10 print-bw-report">
      ...
      <div id="dashboard-export-area" className="bg-[#0e121b] min-h-screen text-slate-200 p-4 lg:p-8 font-sans flex flex-col gap-8 pb-32">
        ...
        <div className="flex justify-between items-center mb-2 relative flex-wrap gap-4">
          <div className={`flex items-center flex-wrap ${activeTab === 'weekly' ? 'justify-center w-full' : 'gap-4'}`}>
              ...
              <div className="flex-1 min-h-0 w-full">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={displayMonthlyData} margin={{top: 30, right: 10, left: 10, bottom: 20}}>
                    ...
                    <Tooltip cursor={{fill: '#1e293b'}} itemStyle={{ color: '#fff' }} contentStyle={{backgroundColor: '#0a0c14', borderColor: '#1f2947', borderRadius: '8px', color: '#fff'}} formatter={(v: any) => gbp(v as any)}/>
              ...
              <div className="flex-1 min-h-0 w-full">
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={displayMonthlyData} margin={{top: 30, right: 10, left: 10, bottom: 20}}>
                    ...
                    <Tooltip cursor={{fill: '#1e293b'}} itemStyle={{ color: '#fff' }} contentStyle={{backgroundColor: '#0a0c14', borderColor: '#1f2947', borderRadius: '8px', color: '#fff'}}/>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
            <div className="bg-[#111520] border border-[#1f2947] rounded-xl w-full max-w-md p-6 shadow-2xl relative">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4">
                  <button onClick={() => {
                    const activeClientId = session?.user?.role === 'admin' ? 'cmpv4dvik0000vdj089wl6zmf' : session?.user?.clientId;
                    setSavingOffer(true)
                    const method = editingOfferId ? 'PUT' : 'POST'
                    const bodyObj = editingOfferId 
                      ? { id: editingOfferId, ...offerForm }
                      : { ...offerForm, clientId: activeClientId, is2025 }
