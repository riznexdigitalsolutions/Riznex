...
- Return ONLY the JSON, no markdown, no explanation.`;
    ...
    if (invoice.type === "pos" && (invoice.platform?.includes("Card") || invoice.fileName?.toLowerCase().includes("card") || invoice.fileName?.toLowerCase().includes("bank") || invoice.fileName?.toLowerCase().includes("walkin"))) {
      ...
      if (extractedText) {
        ...
        const monthsMap: Record<string, number> = { jan:0, feb:1, mar:2, apr:3, jun:5, jul:6, aug:7, sep:8, oct:9, nov:10, dec:11, january:0, february:1, march:2, april:3, may:4, june:5, july:6, august:7, september:8, october:9, november:10, december:11 };

        for (let i = 0; i < lines.length; i++) {
          if (lines[i].includes("US Bank Europe Dac")) {
            let dateStr = "";
    ...
    if (geminiData || (invoice.type === "pos" && extractedText)) {
      ...
      if (invoice.type === "pos") {
        if (geminiData.isHungryBirdsMultiWeek) {
          ...
          const extractTastyBun = (name: string) => {
            ...
            const ordersMatch = lines[lineIdx].match(/Orders.*?(\d+)/i);
            if (ordersMatch) orders = parseInt(ordersMatch[1], 10);
            
            if (lineIdx > 0 && lines[lineIdx - 1].toLowerCase().includes('sales')) {
              const salesMatch = lines[lineIdx - 1].match(/Sales.*?£([0-9,.]+)/i);
              if (salesMatch) sales = parseFloat(salesMatch[1].replace(/,/g, ''));
            } else {
              const salesMatch = lines[lineIdx].match(/Sales.*?£([0-9,.]+)/i);
              if (salesMatch) sales = parseFloat(salesMatch[1].replace(/,/g, ''));
            }
      ...
      if (invoice.type === "supplier") {
        ocrData = parseSupplierInvoice(extractedText, invoice.fileName);
    ...
    if (!finalInvoiceDate && invoice.fileName) {
      ...
      const match = invoice.fileName.match(/(?:(\d{1,2})[_\s]+)?(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*(?:[_\s]+(\d{1,2}))?/i);
      if (match) {
        const monthWord = match[2].toLowerCase();
        let monthIndex = shortMonths.indexOf(monthWord);
        
        let dayStr = match[3];
        if (match[1] && match[3]) {
          dayStr = parseInt(match[3]) <= 31 ? match[3] : match[1];
        } else if (match[1]) {
          dayStr = match[1];
        } else if (match[3]) {
          dayStr = match[3];
        ...
        if (dayStr) {
          ...
          if (monthIndex !== -1 && day >= 1 && day <= 31) {
            finalInvoiceDate = new Date(Date.UTC(2026, monthIndex, day, 12, 0, 0));
            if (!ocrData.invoiceDate) ocrData.invoiceDate = finalInvoiceDate;
    ...
    if (invoice.type === "platform" && invoice.platform) {
      ...
      if (ocrData.isHungryBirdsMultiWeek) {
        ...
        const webAndApp = {
          sales: (ocrData.androweb?.sales || 0) + (ocrData.app?.sales || 0),
          orders: (ocrData.androweb?.orders || 0) + (ocrData.app?.orders || 0)
        };
        await createTastyBunRecord(`${storeName} Web & App`, webAndApp);
      } else {
...
function parseSupplierInvoice(text: string, fileName: string = '') {
  const result: any = {};
  ...
  // Priority 5: JJ Food Service standalone date & time
  const jjDateField = text.match(/(\d{1,2}\/\d{1,2}\/\d{4})\s+\d{2}:\d{2}:\d{2}/);
  ...
  if (invoiceDateSlash) {
    ...
    result.invoiceDate = parseSlashDDMMYYYY(jjDateField[1]);
  ...
  if (!result.invoiceDate && fileName) {
    const fnDateMatch = fileName.match(/(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]*[_\s]*(\d{1,2})/i);
    if (fnDateMatch) {
      let m = fnDateMatch[1].toLowerCase();
      const monMap: any = {jan:'01',feb:'02',mar:'03',apr:'04',may:'05',jun:'06',jul:'07',aug:'08',sep:'09',oct:'10',nov:'11',dec:'12'};
      if (monMap[m]) result.invoiceDate = `2026-${monMap[m]}-${fnDateMatch[2].padStart(2, '0')}`;
    }
  }
  ...
  const jjTotalFormat = text.match(/TOTAL[\s\S]{0,20}?£\s*([\d,]+\.\d{2})/i);
  ...
  if (!result.totalAmount) {
    const poundMatches = [...text.matchAll(/[£$]\s*([\d,]+\.\d{2})/g)];
    if (poundMatches.length > 0) {
      const amounts = poundMatches.map(m => parseFloat(m[1].replace(/,/g, '')));
