          ...
          <table className="w-full text-left text-sm text-slate-300">
            ...
            <tbody className="divide-y divide-[#1f2947]">
              {[...filteredInvoices].sort((a, b) => {
                const aNeedsAttn = ['pending', 'processing', 'error', 'failed'].includes(a.ocrStatus) ? 1 : 0;
                const bNeedsAttn = ['pending', 'processing', 'error', 'failed'].includes(b.ocrStatus) ? 1 : 0;
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-col gap-1.5">
                      {inv.supplier?.name ? (
                        ...
                        <span className="text-red-400 font-bold text-[10px] uppercase tracking-wider border border-red-500/20 bg-red-500/10 px-2 py-0.5 rounded-full w-max">Missing</span>
                      )}
                      {inv.fileName && (
                        <span className="text-slate-500 text-[10px] flex items-center gap-1" title="Original File Name">
