...
export async function POST(req: NextRequest) {
  try {
    ...
    // One Stop Supplier Auto-Fill
    let oneStop = await prisma.supplier.findFirst({
      where: { name: 'One Stop', clientId: actualClientId }
    })
    
    if (!oneStop) {
      oneStop = await prisma.supplier.create({
        data: { name: 'One Stop', clientId: actualClientId, active: true, category: 'food' }
      })
    }

    const existingOneStop = await prisma.invoice.findFirst({
      where: {
        clientId: actualClientId,
        supplierId: oneStop.id,
        type: 'supplier',
    ...
    if (!existingOneStop) {
      await prisma.invoice.create({
        data: {
          ...
          supplierId: oneStop.id,
          type: 'supplier',
          platform: 'Herbies Pizza',
          fileName: 'Auto-filled One Stop (£60)',
          filePath: '/auto-filled',
          fileType: 'auto',
          amount: 60,
          invoiceDate: new Date(date),
          ocrStatus: 'done',
          notes: 'Auto-filled fixed weekly invoice (Herbies Pizza)',
          is2025
        }
      })
    }
