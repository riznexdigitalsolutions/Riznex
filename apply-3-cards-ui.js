...
const anchor = '                      </ComposedChart>\r\n                    </ResponsiveContainer>\r\n                  </div>\r\n                </div>';
const anchorLinux = '                      </ComposedChart>\n                    </ResponsiveContainer>\n                  </div>\n                </div>';
...
if (content.includes(anchor)) {
    content = content.replace(anchor, anchor + uiBlock);
} else if (content.includes(anchorLinux)) {
    content = content.replace(anchorLinux, anchorLinux + uiBlock);
} else {
    console.error("Could not find anchor to inject UI.");
    process.exit(1);
}
